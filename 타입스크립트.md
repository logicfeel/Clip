...
- 타입 호환성
    ...
    - 시작하기
        ...
        - 두 함수 비교
        > - 매개변수는 위치순서와 타입을 기준으로 함 (이름은 무관함)
        >   - 매개변수2 > 매개변수1 : 가능
        >   - 매개변수1 > 매개변수2 : 오류
        > - 리턴타입이 객체인 경우도 같음 
        >   - 리턴객체(2) > 리턴객체(1) : 가능
        ...
        // 강제로 Event 타입을 number 로 변환 => 허용안함
        listenEvent(EventType.Mouse, (e: number) => console.log(e));
        ```

        - 선택적 매개 변수와 나머지 매게 변수
        > `...args`매개변수는 무한한 선택적 매개변수로 처리됨
        ```typescript
    ...
    - 클래스
        ...
        > - private, protected 는 일치해야함
        > - 슈퍼클래스와 가능하면서, 다른 상속 계층의 호환성 제한 가능함

    - 제네릭
    > - 소비(지정)될 때 결과에 영향을 줌
    > - 비어있는 제네릭은 서로 호환됨(내용 있으면 불가)
    > - 제레릭은 소비(타입지정)지정 전에는 any 타입으로 취급함
    ```typescript
    // 빈 제네릭
    interface Empty<T> {
    }
...
고급 주제 #
    ...
    - 고급 주제
        ...
        - 하위 타입 vs 할당
        > - implements 또는 extends 에 따라 결정됨
        > * 자세한 내용은 참조
...
- 고급 타입
    ...
    - 유니온 타입
    > - `원시타입 | 원시타입`은 OR 개념의 유형 (선택)
    > - `타입 | 타입 `은 프로퍼티간의 공집합 개념
    ```typescript
    interface Bird {
        fly(): any;
        layEggs(): any;
